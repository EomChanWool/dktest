                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                            	<form action="${pageContext.request.contextPath}/sl/basicInfo/member/modifyMemberOk.do" name="modifyForm" method="post">
                            		<input type="hidden" name="mIdx" value="${memberVO.mIdx}">
	                                <table class="table table-bordered" id="dataTable">
	                                    <tbody>
											<tr>
												<th>사원명  <span class="req">*</span></th>
												<td><input type="text" class="form-control" name="mName" id="mName" value="${memberVO.mName}"/></td>
												<th>아이디  <span class="req">*</span></th>
												<td><input type="text" class="form-control" name="mId" value="${memberVO.mId}" disabled="disabled"/></td>
											</tr>
											<tr>
											...
											<tr>
												<th>소속</th>
												<td><input type="text" class="form-control" name="mBelong" id="mBelong" value="${memberVO.mBelong}"/></td>
												<th>직급</th>
												<td><input type="text" class="form-control" name="mRank" id="mRank" value="${memberVO.mRank}"/></td>
											</tr>
											<tr>
												<th>레벨</th>
												<td>
													<select name="mLev" id="mLev" class="form-control">
														<option value="1" <c:if test="${memberVO.mLev eq '1'}">selected="selected"</c:if>>현장 작업자</option>
														<option value="2" <c:if test="${memberVO.mLev eq '2'}">selected="selected"</c:if>>관리자</option>
													</select>
												</td>
												<th>입사일</th>
												<td><input type="date" class="form-control" name="mEmpltDte" id="mEmpltDte" value="${memberVO.mEmpltDte}"/></td>
											</tr>
											<tr>
												<th>비고</th>
												<td colspan="3"><textArea name="mNote">${memberVO.mNote}</textArea></td>
											</tr>
										</tbody>
	                                </table>
                                </form>
